...
public sealed partial class OpenAiMcpEditingBridge
{
    ...
    public OpenAiMcpEditingBridge(AiBridgeOptions options, ILogger<OpenAiMcpEditingBridge> logger)
    {
    ...
    public async Task<AiBridgeResult> EditAsync(AiEditRequest request, string userId, CancellationToken cancellationToken)
    {
        ...
        {
            throw new ArgumentException("Prompt is too large. Maximum length is 12000 characters.", nameof(request));
        }

        if (request.AllowMutations && !_options.AllowMutations)
        {
            throw new InvalidOperationException("Mutating AI commands are disabled by MAGMAEDIT_AI_BRIDGE_ALLOW_MUTATIONS.");
        }
        ...
        if (!string.IsNullOrWhiteSpace(request.SessionId))
            mcpTool.Headers["X-MagmaEdit-Session-Id"] = request.SessionId.Trim();
        ...
        return new AiBridgeResult(response.Id, output, request.AllowMutations, response.OutputItems.Count);
    }
    ...
    private static string BuildPrompt(AiEditRequest request)
    {
        ...
        return request.AllowMutations
            ? $"{policy}\n\nThe user has explicitly enabled editing actions for this request. Use the authenticated desktop session selected by the request.\n\nUser request:\n{request.Prompt}"
            : $"{policy}\n\nThis request is read-only. Do not perform mutations. You may inspect editor state and explain what would need to change.\n\nUser request:\n{request.Prompt}";
    }

    [LoggerMessage(EventId = 1000, Level = LogLevel.Information, Message = "Starting AI edit request. UserId={UserId}, MutationsEnabled={MutationsEnabled}, Model={Model}")]
    private partial void LogStarting(string userId, bool mutationsEnabled, string model);

    [LoggerMessage(EventId = 1001, Level = LogLevel.Information, Message = "AI edit request completed. UserId={UserId}, ResponseId={ResponseId}, OutputLength={OutputLength}")]
...
public sealed record AiBridgeResult(string ResponseId, string OutputText, bool MutationsEnabled, int OutputItemCount);
