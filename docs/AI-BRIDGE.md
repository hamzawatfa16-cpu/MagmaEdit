...
The first header authenticates the trusted bridge caller. The second identifies the signed-in MagmaEdit user. The bridge validates the Supabase access token against the project's Auth service before any AI request is started.
...
`allowMutations` is deliberately opt-in. A mutating request also requires the server mutation flag and exactly one configured authorized user ID.
...
The bridge uses the Auth service user lookup rather than trusting user details supplied by the client. Supabase documents `getUser`/`/auth/v1/user` as a server-confirmed way to validate an access token. citeturn755284search1turn755284search3
